...
| [`sicang5`](schemas/cangjie/sicang5/sicang5.schema.yaml)<br>四码仓颉·五代 | `pinyin_simp` | 纯单字四码仓颉，由仓颉五代码表按四码规则（一二三末码）提取生成，默认 `150` 个二简、`300` 个三简；一简使用单独校准表，二简/三简按净收益生成，二简保护高频 GB2312 原生二码字，三简保护原生三码位，全码采用简码让位，已有简码字让位给同码且达到常用门槛的无简码字。最长四码不自动上屏，不支持整句连打；支持 `z` 字根码、`z` 后缀选重、拼音反查、扩展字符集和简繁转换。 |
| [`wucang5`](schemas/cangjie/wucang5/wucang5.schema.yaml)<br>五码仓颉·五代 | `pinyin_simp` | 默认启用。基于仓颉五代码表生成的纯单字五码方案，默认 `150` 个二简、`300` 个三简，不启用四简；一简使用单独校准表，二简/三简按净收益生成，二简保护高频 GB2312 原生二码字，三简保护原生三码位，全码采用简码让位，已有简码字让位给同码且达到常用门槛的无简码字。支持 `z` 字根码、`z` 后缀选重、拼音反查、扩展字符集和简繁转换。五码不自动上屏；输入 `` `拼音 `` 可拼音反查。 |
...
| [`zrm_single`](schemas/shuangpin/zrm_single/zrm_single.schema.yaml)<br>自然码·仓颉·单字 | `pinyin_simp` | 默认启用。纯单字自然码音形方案，不收词、不造词；一位辅助码末尾补 `z`，单字短码按分组首选和高频次选生成；输入 `o` 加完整仓颉五代码作隔离兜底，同字多码全部保留。 |
...
完整、可机器读取的文件级依赖由 [`schemas/common/dependencies.yaml`](schemas/common/dependencies.yaml) 生成并维护，包含 schema、dict、词频文件、语法模型、OpenCC、Lua、符号预设和外部预设。导出方案时以该文件对应的脚本扫描结果为准。

## 共用资源

| 资源 | 用途 |
| --- | --- |
| [`opencc`](schemas/common/opencc) | 简繁转换配置和词典。 |
| [`symbols.yaml`](schemas/common/symbols.yaml) | 符号输入预设。 |
| [`lua/aux_code_filter.lua`](schemas/common/lua/aux_code_filter.lua) | 通用辅助码过滤器，目前用于 `pinyin_ice_cangjie5`。 |

## 平台配置

| 文件 | 用途 |
| --- | --- |
| [`default.yaml`](schemas/common/default.yaml) | 桌面/通用默认配置。 |
| [`default.custom.yaml`](schemas/common/default.custom.yaml) | 移动端覆盖配置，目前用于移动端软键盘通过 `backslash` 快速切换到下一个方案。 |

## 脚本

| 脚本 | 用途 |
| --- | --- |
| [`scripts/cangjie/core/cangjie_builder.py`](scripts/cangjie/core/cangjie_builder.py) | 从仓颉五代码表按指定长度规则取码生成字典的核心提取模块。 |
| [`scripts/cangjie/gen_sancang5.py`](scripts/cangjie/gen_sancang5.py) | 基于 `cangjie_builder.py`，生成三码仓颉五代码表（sancang5）。可选生成带词语的便携码表。 |
| [`scripts/cangjie/gen_sicang5.py`](scripts/cangjie/gen_sicang5.py) | 基于 `cangjie_builder.py`，生成带受保护二简、三简和后缀选重的纯单字四码仓颉码表（sicang5）。 |
| [`scripts/cangjie/gen_wucang5.py`](scripts/cangjie/gen_wucang5.py) | 基于 `cangjie_builder.py`，生成带受保护二简、三简和后缀选重的纯单字五码仓颉码表（wucang5）。 |
