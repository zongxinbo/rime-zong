...
| Schema | 名称 |
| --- | --- |
| [`sancang5`](sancang5/sancang5.schema.yaml) | 三码仓颉·五代 |
...
依赖列对应各 schema 中的 `dependencies`，主要用于反查或混合输入。完整文件级依赖见 [`dependencies.yaml`](dependencies.yaml)。
...
完整、可机器读取的文件级依赖由 [`dependencies.yaml`](dependencies.yaml) 生成并维护，包含 schema、dict、词频文件、语法模型、OpenCC、Lua、符号预设和外部预设。导出方案时以该文件对应的脚本扫描结果为准。
