...
- 单字：原型为双拼 2 码 + 仓颉五代首尾辅助码 2 码；最终码表显式展开为一/二/三/四码多编码条目，少量异读可白名单补双拼二码。
- 词语：同时生成逐字首码短码、全双拼码，以及两条路线各自的辅码定重码。
- 仓颉：完整仓颉码前加 `o`，候选排在普通条目之后；如果仓颉短码被普通候选挡住，且补码后不超过 6 码，则另补 `z` 尾码直达。
...
词语同时生成短码路线和全码路线：短码使用逐字双拼首码，并可追加首末字仓颉辅码；全码使用逐字双拼全码，也可追加首末字仓颉辅码。
...
默认词源使用 `schemas/pinyin_ice/pinyin_ice.base.dict.yaml`。默认阈值按词长区分：二字词权重不低于 10，三字和四字词权重不低于 3000；同时过滤以语气词、结构助词、轻助动词和连接虚词组成的语流片段。它比 `pinyin_simp` 更现代，也更适合当前这种轻词库音形方案。
