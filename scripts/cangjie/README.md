...
- 前缀选重：Sicang5/Wucang5 默认共享 `z?/x?`、`z??/x??` 高频前缀短码；Sicang5 用 `z???/x???` 处理四码第 2/3 候选，Wucang5 用 `z???/x???` 生成四码前缀短码，并预留 `z????/x????` 五码第 2/3 候选救援；二码前缀对标一简用 `sc`，三码及更深层前缀用 `sc_balanced`
...
- `--dedup-prefix-level3-weights`：三码前缀权重，默认 `sc_balanced`。
- `--dedup-prefix-full-weights`：四码前缀权重，默认 `sc_balanced`。
