...
`a-z` 一简的静态排序只使用日常字频和记忆锚点，不加入码长、重码或前缀占位救援权重。主推荐默认使用 `--objective mnemonic`，即锚点等级优先；同锚点内允许净收益达到 2,000,000 的候选突破日常频率排序，并优先考虑省键数更大的候选。`--objective hybrid` 会在同锚点等级内优先真实净收益，`--objective gain` 用于收益最大化审计。`x/z` 没有普通仓颉锚点，使用 `--append-xz` 追加时会在普通位定案后重新生成全局候选，并固定按真实净收益优先分配。真实 S2/S3 重放仍用于展示副作用和排除负收益替换；普通二三四简和自动消重层保留重码救援评分。
...
- 三简：固定 `1300` 个；默认保护高频 GBK 原生三码位，综合字频门槛 `3000`
...
- 简码总量：builder 口径含 `root_code` 字根/特殊键本体码 30 个、一简 26 个、二简 300 个、三简 1300 个，默认合计 `1656` 个
...
- 前缀选重：Sicang5/Wucang5 默认按各自码长独立生成 `z?/x?`、`z??/x??` 高频前缀短码；所有 `z/x` 前缀均按候选位固定映射，第 2 候选用 `z`，第 3 候选用 `x`；Sicang5 用 `z???/x???` 处理四码第 2/3 候选，Wucang5 用 `z???/x???` 生成四码前缀短码，并预留 `z????/x????` 五码第 2/3 候选救援；二码前缀对标一简用 `sc`，三码及更深层前缀用 `sc_daily`
- 后缀选重：前缀层生成后再兜底；Sicang5/Wucang5 默认第 2 候选补 `z`、第 3 候选补 `x`；Sicang5 处理源码长度 `<= 3`，Wucang5 处理源码长度 `<= 4`

二三简数量评估策略：

- 判断二简、三简自身的合理数量时，用 `core/evaluate_shortcut_counts.py --dedup-layers none`，只评估字根、一简、二简、三简和全码，不让 `z/x` 前缀、后缀消重层兜底。
- 验证最终用户体验时，用 `--dedup-layers all` 或直接运行 `scripts/assess/summary.py`，该口径包含自动前缀和后缀消重层。
- 当前按 `summary.py` 的繁简联合口径、`sc_daily`、原生码位保护门槛 `3000`、长码候选门槛 `3000` 扫描，二简 `300/310` 在三简 `1300` 时动态选重率相同；默认保留二简 `300`，三简采用 `1300`。
...
- 二码和三码前缀也按方案独立生成：Sicang5 使用四码投影基线，Wucang5 使用五码实际候选基线。
...
- 构建时会覆盖生成方案专属的 `prototypes/prefix_code_2_sicang5.txt`、`prefix_code_3_sicang5.txt`、`prefix_code_2_wucang5.txt`、`prefix_code_3_wucang5.txt`、`prefix_code_4_sicang5.txt`、`prefix_code_4_wucang5.txt`、`prefix_code_5_wucang5.txt`，以及后缀审阅文件 `suffix_code_sicang5.txt`、`suffix_code_wucang5.txt`；这些文件不是人工输入源。
...
- `--dedup-prefix-source-max-code-length`：前缀生成使用的投影基线码长；Sicang5 默认 `4`，Wucang5 默认 `5`。
