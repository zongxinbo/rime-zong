...
简体/通用一简与繁体专精一简分表维护：`one_code.txt` 服务当前通用方案；需要繁体专精时生成并人工定稿 `one_code_tc.txt`，构建繁体方案时替换主一简层。`z?`、`x?` 不承载繁体高频补位；固定前缀层当前不进默认构建，只作为以后人工补丁或实验入口。
...
- 前缀选重：自然 `z/x` 前缀和 `z?/x?` 固定前缀默认关闭；需要实验时显式开启
...
# 显式开启自然 z/x 前缀直达码
python scripts/cangjie/gen_sicang5.py --dedup-prefix
