...
若某字已在 `z_code.txt` 中定义为字根字，最终字典会过滤该字的一报码入口，让字根字统一通过 `az/azz/bz...` 这类 z 字根码输入。
...
`最终简码条目` 使用 builder 口径，包含 `z_code` 字根码、有效一简、二简、三简：`31 + 23 + 150 + 300 = 504`。`one_code.txt` 中与 `z_code.txt` 重复的字根字不进入最终一报码。

当前 `summary.py` 摘要：

| 语料 | 频率降序-全码 | 频率降序-简码 | 原始码表-全码 | 原始码表-简码 | 简全联用-实际 |
| :--- | ---: | ---: | ---: | ---: | ---: |
| 知乎简体 | 6.84‱ | 68.17‱ | 7.21‱ | 82.53‱ | 7.21‱ |
| 北语简体 | 5.60‱ | 83.14‱ | 5.64‱ | 95.65‱ | 5.64‱ |
| 台标繁体 | 2.28‱ | 46.03‱ | 22.90‱ | 199.51‱ | 21.71‱ |
| 古籍繁体 | 4.97‱ | 70.94‱ | 21.65‱ | 273.11‱ | 20.43‱ |
| 繁简联合 | 10.13‱ | 97.79‱ | 19.05‱ | 164.06‱ | 18.23‱ |
