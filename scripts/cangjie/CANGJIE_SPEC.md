...
## 2. 核心数据源
- **Dialogue 口语语料 (`schemas/common/frequency/char/dialogue_char_freq.txt`)**：核心口语字频（源自北语大语料库加工），确保高频对话词汇优先。
- **Subtlex 影视字幕语料 (`schemas/common/frequency/char/subtlex_char_freq.txt`)**：基于影视字幕的日常频次，反映口语视听习惯，覆盖常用生活用语。
- **知乎语料 (`schemas/common/frequency/char/zhihu_char_freq.txt`)**：现代网络常用语料，提供更具时代感的频次分布。
- **北语大语料 (`schemas/common/frequency/char/blcu_char_freq.txt`)**：超大规模平衡语料库，提供基础字频基底。
- **Essay 语料 (`schemas/common/frequency/word/essay-zh-hans.txt`)**：传统 Rime 基础语料库。
