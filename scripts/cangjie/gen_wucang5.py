...
def main():
    ...
    parser.add_argument("--suffix-z-ranks", default="2:z,3:x",
                        help="z/x 后缀候选位规则，格式如 2:z,3:x；Wucang5 默认第 2 候选补 z、第 3 候选补 x")
    parser.add_argument("--suffix-z-max-source-length", type=int, default=4,
                        help="z/x 后缀只处理不超过该长度的源码；Wucang5 默认 4")
    ...
    parser.add_argument("--dedup-prefix-short", action=argparse.BooleanOptionalAction, default=True,
                        help="是否生成 z?/x?、z??/x?? 高频前缀短码（默认开启；需 --dedup-prefix 生效）")
    parser.add_argument("--dedup-prefix-full", action=argparse.BooleanOptionalAction, default=True,
                        help="是否为四码全码第 2/3 候选生成 z前三/x前三 前缀选重码（默认开启；需 --dedup-prefix 生效）")
    ...
    generate_dict(
        ...
        suffix_code_path=SUFFIX_CODE_WUCANG5_PATH,
        ...
        dedup_prefix_scheme="wucang5",
        dedup_prefix_scheme_short_levels=(4,),
        dedup_prefix_scheme_full_source_length=5,
